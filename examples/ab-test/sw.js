...
const rewriteForTesting = (experiments1)=>(event1, response)=>Promise.resolve({
        }).then(_addExperimentVariations(experiments1, event1.request)).then(_addUserId(event1.request)).then(_transformForTesting(new HTMLRewriter(), sendToElastic, event1, response)).then(_addExperimentCookies).then(_addUserCookie).then(({ response: response1  })=>response1
        )
;
const rewriteForTesting1 = rewriteForTesting;
const getAsset = async ({ request  })=>{
